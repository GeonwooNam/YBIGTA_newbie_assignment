...
def embed_passages(texts: list[str], ids: list[str], progress_callback=None) -> np.ndarray:
    ...
    def process_batch(client_idx, batch_start, batch_texts):
        client = clients[client_idx % len(clients)]
        vecs = _embed_batch_safe(client, batch_texts)
        with lock:
            for j, vec in enumerate(vecs):
                embeddings[batch_start + j] = vec
            done[0] += len(batch_texts)
            if progress_callback:
                progress_callback(done[0], n)

    with ThreadPoolExecutor(max_workers=len(clients)) as executor:
        futures = []
        for idx, (start, batch_texts) in enumerate(batches):
            futures.append(executor.submit(process_batch, idx, start, batch_texts))
            time.sleep(MIN_INTERVAL)
